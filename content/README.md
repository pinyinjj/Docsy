...
- **Mermaid 图表渲染（强制规范）**：
  - **原则**：为了确保在磨砂玻璃背景下的易读性及大尺寸展示效果，**禁止**直接使用 Markdown 原生的 ` ```mermaid ` 代码块。
  - **规范**：
    必须使用自定义的 `mermaid` Shortcode 进行包裹。
  - **语法示例**：
    ```markdown
...
{{< /mermaid >}}
  ...
  - **尺寸参数 (`size`)**：
    - `xs`: 占 20% 宽度。
    - `sm`: 占 35% 宽度（**规范：适用于类图 `classDiagram`, `graph` 等纵向窄图**）。
    - `md`: 占 65% 宽度。
    - `lg`: 占 100% 宽度（**默认：适用于`sequenceDiagram`、关联关系图等复杂逻辑**）。
    - `xl`: 占 115% 宽度（略微溢出文字列，适合超复杂架构图）。
    - `full`: 占 130% 宽度（最大化利用屏幕，适合超大型流程图）。
  - **样式特性**：Shortcode 会自动为图表添加纯白背景、内边距、圆角阴影并保持左对齐。
  - **解析限制（重要）**：
    - **空行隔离**：必须在 `{{< mermaid >}}` 标签的前后各留出一个完整的空行，以确保 Hugo 编译器能正确识别短代码。
    - **禁止行内杂质**：标签 `{{< mermaid >}}` 和 `{{< /mermaid >}}` 必须独占一行，且行内严禁包含任何多余的空格、制表符或其他不可见字符。
