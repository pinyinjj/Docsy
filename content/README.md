...
- **逗号与反引号使用规范（强制）**：在正文文本中，应该使用逗号（，）的地方，不能使用反引号（`）这个符号。反引号（`）应该仅用于标记代码、命令、文件名等技术术语，而不应该替代标点符号使用，否则会导致显示效果很难看。
  - 正确示例：这是一个示例，展示了正确的用法。你可以使用 `hugo server` 命令启动服务器，然后查看效果。
  - 错误示例：这是一个示例`展示了错误的用法。你可以使用 `hugo server` 命令启动服务器`然后查看效果。
