...
- **交互元素与突出显示（强制）**：按钮、菜单项、标签页、选项等交互类元素或需要突出显示的文本，必须使用反引号（`` ` ``）来突出显示，禁止使用双引号（`"`）。
  ...
  - 错误示例：点击 `Open In Browser` 按钮、选择 `Project` 分页（反引号后不能使用顿号）
- 反引号也用于突出显示文件名、路径、配置项名称、技术术语等需要强调的文本。
- **逗号、反引号与加粗使用规范（强制）**：
  - **反引号（`` ` ``）的正确用途**：在 Markdown 中，反引号用于创建行内代码高亮，应该仅用于标记以下内容：
    - 代码片段：`const x = 1;`
  ...
  - **加粗（`**...**`）的正确与错误用法**：
    - 正确：用于强调一段完整的词组或术语，例如：**动捕系统**，**伴随计算机**，**飞控**
    - 错误：在加粗标记之后直接使用中文顿号连接多个加粗短语，例如：**动捕系统**、**伴随计算机**、**飞控**
    - 当需要列举多个加粗项时，应使用逗号（，）或英文逗号（,）分隔，加粗范围只包裹具体词语本身，而不包裹标点
  - **关键原则**：
    - 反引号是 Markdown 的语法标记，用于高亮显示，不是标点符号。所有中文标点符号（，。、；：等）和英文标点符号（, . ; : 等）都不能用反引号替代。
    - **反引号后禁止使用中文顿号（、）**：当需要列举多个用反引号标记的项时，应使用逗号（，）或英文逗号（,）分隔，不能使用顿号（、），否则会导致显示异常。
